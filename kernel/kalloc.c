...
static void kfree_cpu(void *pa, int id)
{
    ...
    memset(pa, 1, PGSIZE); // 填充垃圾以捕获悬空引用。
...
void freerange(void *pa_start, void *pa_end)
{
    ...
    for (; p + PGSIZE <= (char *)pa_end; p += PGSIZE)
    {
        if (((uint64)p % PGSIZE) != 0 || (char *)p < end || (uint64)p >= PHYSTOP)
            panic("freerange"); // 基本健壮性检查（与原 kfree 同步）

        kfree_cpu((void *)p, id); // 基本健壮性检查（与原 kfree 同步）
        id = (id + 1) % NCPU;
    }
}
...
void *kalloc(void)
{
    ...
    push_off();  // 获取当前 CPU id（关中断使用）
    int id = cpuid();
    pop_off();

    acquire(&kmems[id].lock);  // 先尝试本地 freelist
    r = kmems[id].freelist;
    if (r)
